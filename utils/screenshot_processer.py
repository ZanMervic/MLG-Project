...
def process_image(path, max_color_dist=100):
    ...
    yellow = (238, 223, 80) # 156, 147, 66
    # Loop through all pixels forward
    flag = False
    for x in range(width):
        for y in range(int(0.5 * height)):
            rgb = img.getpixel((x, y))
            if color_distance(rgb, yellow) < 185 and rgb[0] - rgb[1] < 25 and rgb[0] - rgb[2] > 80:
                f = True
                for j in range(10):
                    yj = y + j
                    rgb = img.getpixel((x, yj))
                    if not(color_distance(rgb, yellow) < 185 and rgb[0] - rgb[1] < 25 and rgb[0] - rgb[2] > 80):
                        f = False
                if f:
                    upper_left = (x, y)
                    flag = True 
    ...
    for x in range(width)[::-1]:
        for y in range(int(0.5 * height), height)[::-1]:
            rgb = img.getpixel((x, y))
            if color_distance(rgb, yellow) < 185 and rgb[0] - rgb[1] < 25 and rgb[0] - rgb[2] > 80:
                f = True
                for j in range(10):
    ...
    for i, a in enumerate("ABCDEFGHIJK"):
        for j, n in enumerate(range(1, 19)[::-1]):
            ...
            green_p = sum([p for color, p in top_n_colors if color_distance(color, green) < max_color_dist and color[1] - color[0] > 50])
            if red_p > 0.17:
...
if __name__ == "__main__":
    data = os.listdir(os.path.join("..", "screenshots"))
